...
const Download = () => {
  const [fileList, setFileList] = useState([
    ...
    },
    {
      ...
      size: "2.5 MB",
      description:
        "An introductory guide to the fundamental principles of UX design.",
      status: "canceled",
      progress: 0,
    },
  ]);
  ...
  const startDownload = (id) => {
    setFileList((prev) =>
      prev.map((prevData) =>
        prevData.id === id && prevData.status === "canceled"
          ? { ...prevData, status: "downloading", progress: 0 }
          : prevData
      )
    );
  ...
  const cancelDownload = (id) => {
    setFileList((prevFiles) =>
      prevFiles.map((prevData) =>
        prevData.id === id
          ? { ...prevData, status: "canceled", progress: 0 }
          : prevData
      )
    );
  };
    ...
    <section className=" bg-white rounded shadow-lg lg:min-w-[449px]  h-[188px] my-5 lg:my-0 lg:ms-6 ">
      <div className="px-5 py-2 max-w-[407px] h-[153px] mx-auto">
        ...
        <div className="text-center">
          <button className="text-primary bg-primary/5 w-full h-5 rounded-md hover:bg-primary/15 duration-300 hover:scale-105 text-xs font-medium">
            see more
          </button>
        </div>
      </div>
