...
const initialNotes = [
  {
    ...
    `,
    des2: "* Introduction to Newest Product Plan",
    des3: "* Monthly Revenue updates for each products",
    type: "Product",
  },
  {
    id: 2,
    ...
    <div className="px-2 lg:px-6 pb-6">
      <div className="flex flex-col lg:flex-row justify-between border mb-7 px-3 items-center">
        ...
        <div className="flex flex-col lg:flex-row items-center gap-4 my-3">
          ...
          <button
            className="bg-primary text-xs lg:text-base py-1.5 px-3 rounded text-white"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 ">
        {filteredNotes.map((note) => (
          <div
            key={note.id}
            className="border border-[#E4E4E4] rounded-lg shadow-sm  max-w-[319px] min-h-[203px] "
          >
            <div className="ps-[22px] mr-4 mt-4">
              <div className="flex items-center gap-[11px] ">
                <button className="bg-[#F7F7E8] px-[5px] py-[3px] rounded text-[#B1AB1D] font-medium">
                  {" "}
                  {note.category}{" "}
                </button>
                <button className="bg-[#EDF2FE] px-[5px] py-[3px] rounded text-[#3083FF] font-medium">
                  {" "}
                  {note.type}{" "}
                </button>
              </div>
              <div className="">
                <h3 className="text-[14px] py-[10px] font-semibold relative">
                  {note.title}
                </h3>
                <p className="text-[12px] pr-5 leading-[18px] text-[#727272] ">
                  {note.des} <br />
            ...
            <div className="flex items-center justify-between border-t pt-2">
              ...
              <p className="pr-[22px] text-xs font-medium  text-[#727272] ">
                {note.date}
              </p>
            </div>
          </div>
