...
const TodoList = () => {
  ...
  return (
    <div className="min-w-[288px] mt-4 bg-white shadow-cardShadow rounded-md border border-[#E2E8F0] lg:ms-7">
      <h1 className="text-[15px] font-semibold leading-[23px] pt-[11px] ps-[28px]">
        To do List
      </h1>
      <ul className="space-y-2 pb-6">
        {tasks.map((task) => (
          <li
            key={task.id}
            className="flex items-center gap-4  pt-[10px] border-b-[1.5px] pb-2 border-[#ECECEC] mx-6"
          >
            <div
            ...
            >
              {task.completed && <img src="/Line.png" alt="" />}
            </div>
            <div>
              <p
                className={`text-[13px] font-medium  ${
                  task.completed
                    ? "line-through text-[#41475E]"
                    : "text-[#121212]"
                }`}
              >
                {task.title}
